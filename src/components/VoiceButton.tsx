...
export default function VoiceButton({ onCreated }: { onCreated?: (result: Result) => void }) {
  ...
  const toastContent = state === "listening"
    ? { bg: "bg-coral text-white", text: "🎤 Listening… tap to stop" }
    : state === "processing"
    ? { bg: "bg-canvas border border-hairline text-ink-muted", text: transcript ? `"${transcript}"` : "Processing…" }
    : state === "done" && result
    ? { bg: "bg-green-500 text-white", text: `✓ ${result.type} created!` }
    : state === "error"
    ? { bg: "bg-red-500 text-white", text: errorMsg }
    : null;

  return (
    <>
      {/* Fixed toast — always visible regardless of overflow */}
      {toastContent && (
        <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] rounded-full px-4 py-2 text-sm font-medium shadow-lg ${toastContent.bg}`}>
          {toastContent.text}
        </div>
      )}
