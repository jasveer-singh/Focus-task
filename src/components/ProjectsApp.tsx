    ...
    >
      <div className="w-full max-w-lg rounded-xl border border-hairline bg-white flex flex-col max-h-[90vh] animate-fade">
        <div className="flex items-center justify-between border-b border-hairline px-6 py-4">
          <h2 className="font-display text-xl font-normal tracking-[-0.3px] text-ink">
        ...
        <form
          className="overflow-y-auto flex flex-col gap-4 px-6 py-5"
          onSubmit={(e) => { e.preventDefault(); if (title.trim()) { onSave(title.trim(), desc.trim(), status); onClose(); } }}
        >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              className="rounded-md border border-hairline bg-canvas px-3 py-2.5 text-sm text-ink outline-none transition placeholder:text-ink-soft focus:border-coral resize-none"
            />
          </div>
          <div className="flex flex-col gap-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["planning", "active", "on-hold", "completed"] as ProjectStatus[]).map((s) => (
                <button
                  ...
                  type="button"
                  onClick={() => setStatus(s)}
                  className={`rounded-md border py-2 text-sm font-medium capitalize transition ${
                    status === s
...
// ── Add Task Modal (within a project) ─────────────────────────────────────────
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg rounded-xl border border-hairline bg-white flex flex-col max-h-[90vh] animate-fade">
        ...
        <form
          className="overflow-y-auto flex flex-col gap-4 px-6 py-5"
          onSubmit={(e) => { e.preventDefault(); if (title.trim()) { onSave(title.trim(), notes.trim(), dueAt ? new Date(dueAt).toISOString() : null); onClose(); } }}
        >
          <div className="flex flex-col gap-1.5">
...
function ProjectCard({ project, taskCount, onSelect, onDelete }: { project: Project; taskCount: number; onSelect: () => void; onDelete: () => void }) {
  ...
  const completedCount = 0; // could be derived if needed

  return (
    <button
      type="button"
      onClick={onSelect}
      className="group relative w-full rounded-xl border border-hairline bg-canvas p-5 text-left transition hover:border-coral/40 hover:shadow-sm animate-rise"
    >
      {/* Status pill */}
      <span className={`inline-flex items-center rounded-pill px-2 py-0.5 text-[10px] font-medium uppercase tracking-[1px] ${meta.pill}`}>
        {meta.label}
      </span>
      ...
      {project.description && (
        <p className="mt-1 text-sm text-ink-muted leading-relaxed line-clamp-2">{project.description}</p>
      )}
      ...
      <button
        ...
        className="absolute right-3 top-3 hidden rounded-md border border-transparent p-1 text-xs text-ink-soft transition hover:border-hairline hover:text-coral group-hover:flex"
      >
      ...
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-hairline pb-8">
        <div>
          ...
          <p className="mt-3 max-w-xl text-sm text-ink-muted leading-relaxed">
            Organise related tasks into projects. Tasks added here also appear in your Tasks view.
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="flex flex-col items-center gap-4 py-20 text-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-full border border-dashed border-hairline">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
              <rect x="2" y="4" width="16" height="13" rx="2" stroke="#cc785c" strokeWidth="1.4"/>
              <path d="M6 4V3a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v1M7 9h6M7 12h4" stroke="#cc785c" strokeWidth="1.4" strokeLinecap="round"/>
            </svg>
          </div>
    ...
    <section className="flex w-full flex-col gap-0 px-8 py-10 lg:px-10">
      ...
      <div className="mb-6">
        <button
          type="button"
          onClick={onBack}
          className="flex items-center gap-1.5 text-xs text-ink-muted transition hover:text-coral mb-4"
        >
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-hairline pb-6">
          <div>
            ...
            {project.description && (
              <p className="mt-2 max-w-xl text-sm text-ink-muted leading-relaxed">{project.description}</p>
            )}
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="rounded-md border border-hairline px-3 py-1.5 text-xs font-medium text-ink-soft transition hover:border-hairline hover:text-coral"
            >
              Delete
            </button>
          </div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-md bg-coral px-3 py-1.5 text-xs font-medium text-white transition hover:bg-coral-active"
        >
        ...
        <div className="flex flex-col gap-2">
          {tasks.map((task, index) => {
            ...
            return (
              <article
                key={task.id}
                className={`rounded-lg border bg-canvas p-4 transition animate-rise ${overdue ? "border-coral/30" : "border-hairline"} ${task.completed ? "opacity-60" : ""}`}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className={`text-sm font-medium text-ink leading-snug ${task.completed ? "line-through text-ink-soft" : ""}`}>
                        {task.title}
                      </p>
                      ...
                      {task.notes && !isEditing && (
                        <button
                          ...
                          className="mt-1 text-xs text-ink-soft hover:text-ink transition"
                        >
...
export default function ProjectsApp({
  ...
  // Sync task updates from other tabs / components
  useEffect(() => {
    function onUpdate() {
      try { setTasks(JSON.parse(localStorage.getItem(TASKS_KEY) || "[]")); } catch { setTasks([]); }
    }
  ...
  function deleteProject(id: string) {
    ...
    saveTasks(tasks.map((t) => (t.projectId === id ? { ...t, projectId: null } : t)));
    if (selectedId === id) setSelectedId(null);
  }

  // Task CRUD
  function addTask(title: string, notes: string, dueAt: string | null) {
    if (!selectedId) return;
    const t: Task = { id: buildId(), title, notes, completed: false, pinned: false, dueAt, createdAt: Date.now(), projectId: selectedId };
    saveTasks([t, ...tasks]);
  }
  ...
  function editTask(id: string, title: string, notes: string, dueAt: string | null) {
    saveTasks(tasks.map((t) => (t.id === id ? { ...t, title, notes, dueAt } : t)));
  }
